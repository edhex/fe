(function(){
  ...
  // Mobile navigation uses native <details> (2026-08-18F), so opening never depends on JavaScript.
  // JavaScript only adds quality-of-life behaviour: body scroll lock, Escape and close-on-link.
  const mobileNav=document.getElementById('mobileNav');
  if(mobileNav){
    const syncMenuState=function(){
      document.body.classList.toggle('menu-open',mobileNav.open);
      const summary=mobileNav.querySelector('summary');
      if(summary) summary.setAttribute('aria-label',mobileNav.open?'Close navigation menu':'Open navigation menu');
    };
    mobileNav.addEventListener('toggle',syncMenuState);
    mobileNav.querySelectorAll('a').forEach(function(link){
      link.addEventListener('click',function(){ mobileNav.open=false; syncMenuState(); });
    });
    document.addEventListener('keydown',function(e){
      if(e.key==='Escape'&&mobileNav.open){ mobileNav.open=false; syncMenuState(); mobileNav.querySelector('summary')?.focus(); }
    });
    window.addEventListener('resize',function(){
      if(window.innerWidth>720&&mobileNav.open){ mobileNav.open=false; syncMenuState(); }
    });
    syncMenuState();
  }

  // Avoid covering the opening hero with the fixed mobile CTA.
  function updateVisualViewportBottom(){
    ...
    document.documentElement.style.setProperty('--visual-viewport-bottom',gap+'px');
  }
  function updateMobileCta(){
    document.body.classList.toggle('mobile-cta-ready',window.innerWidth<=720 && window.scrollY>360);
  ...
  const valueData={
    access:{kicker:'Rare access',title:'Borrow the experience before you need it.',copy:'A student may never otherwise get to sit down with people who have just achieved exceptional ATAR results and ask them the questions that usually stay unanswered. FutureEdge makes that access simple and brings more than one perspective into the room.'},
    unfiltered:{kicker:'The turning points',title:'What changed when their first approach stopped working.',copy:'The useful lessons are often in the adjustments. The routine they dropped, the advice they finally listened to, the moment they changed how they revised, or the way they reset when pressure built. FutureEdge pulls out those turning points so students can adjust earlier.'},
    curated:{kicker:'Patterns across many voices',title:'One person gives you a method. Many reveal what repeats.',copy:'FutureEdge compares answers across a growing group of high performers and education voices. When the same behaviours, decisions or warnings keep showing up, they are more useful than a single success story. Those patterns shape the resources.'}
  };
  ...
  const outcomes={
    student:{title:'Get the useful lessons earlier.',rows:[
    ...
    parent:{title:'Support without becoming the study police.',rows:[
      ['Know what actually helps','What students say parents did that made hard weeks easier and performance better.'],
  ...
  const questionData={
    assessment:{title:'After a bad assessment, the best students usually reset fast.',note:'Skip the polished story. We want the actual moves: what they changed next, what they stopped doing and how they got perspective back.',examples:[['Diagnose the real issue','Was it knowledge, timing, exam technique, stress or overconfidence?'],['Change something immediately','High performers usually changed the system, not just promised to “work harder”.'],['Recover perspective quickly','One bad result became feedback, not an identity crisis.']]},
    ...
    plan:{title:'The original plan is often wrong. Good students notice early.',note:'One of the most useful lessons is how people recognised a system was failing before it became expensive.',examples:[['Watch the signals','Low recall, poor practice scores, or too many hours for too little gain.'],['Adapt by subject','What works for English may not work for Methods.'],['Protect energy','A “perfect” plan that is impossible to sustain is still a bad plan.']]},
  ...
  function activateVoice(card){
    voiceCards.forEach(function(c){c.classList.toggle('is-active',c===card);});
  }
  const initialVoice=document.querySelector('.voice-card[data-voice="chris"]')||voiceCards[0];
  if(initialVoice) activateVoice(initialVoice);
  voiceCards.forEach(function(card){
    function toggle(){activateVoice(card);card.classList.toggle('open');}
    card.addEventListener('mouseenter',function(){activateVoice(card);});
    card.addEventListener('focus',function(){activateVoice(card);});
  ...
  const audienceData={
    student:[
      ...
      ['Many perspectives in one place','Useful patterns drawn from a wider group, not just one person’s method.']
    ],
    parent:[
      ['Support without hovering','What students say helped from home, and where well-meant involvement became pressure.'],
      ['Better questions to ask','Prompts that create useful conversations without turning every dinner into a study status meeting.'],
  ...
  form.addEventListener('submit',async function(e){
    ...
    payload._subject='FutureEdge ATAR Success Series: Priority List';
